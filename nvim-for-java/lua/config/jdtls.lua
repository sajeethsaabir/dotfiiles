local function getjdtls()
	-- Get Mason Registry to gain access  to downloaded binaries
	local mason_registry = require("mason-registry")

	-- Find the JDTLS package in the mason Registry
	local jedtls = mason_registry.get_package("jdtls")

	-- Find the full path  to the directory where Masonhas downloaded the JDTLS binaries
	local jdtls_path = jdtls:get_install_path()

	-- Obtain the path to the jar which  runs the lanhuage server
	local laumcher = vim.fn.glob(jdtls_path .. "/plugins/org.eclipse.equinox.launcher_*.jar")

	-- Declare which Operating System is used, Windows use: win, mac use:mac
	local SYSTEM = "linux"

	-- Obtain the path to configuration files for the specific OS
